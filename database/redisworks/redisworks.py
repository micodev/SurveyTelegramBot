...
OTHER_STANDARD_TYPES = {'Decimal': Decimal,
                        'datetime': datetime.datetime, 'date': datetime.date, 'nonetype': None}
...
class Root(Dot):
    ...
    def doformat(value, the_type=None, force_serialize=False):
        ...
        if the_type:
            ...
            if force_serialize:
                ...
                value = {Root.doformat(i, force_serialize=True): Root.doformat(
                    value[i], force_serialize=True) for i in value}
                return value
        ...
        value = TYPE_FORMATS[the_type].format(
            actual_type=value.__class__.__name__, value=new_value)
    ...
    def get_str(self, value):
        if value.startswith(bTYPE_IDENTIFIER):
            ...
            _, actual_type, value = value.split(bITEM_DIVIDER)
        ...
        for i, value in enumerate(values):
            ...
            if value is None:
                ...
                if redis_type in (None, b'none'):
                    ...
                    value = {self.get_str(i): self.get_str(
